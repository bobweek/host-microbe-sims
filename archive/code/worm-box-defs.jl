...
@with_kw mutable struct Communittee
...
@with_kw mutable struct System
	...
	maxFrac::Float64 = 0.1	# maximum fraction of Environment microbes acquired

	# system components
	E::Environment			# microbe abundances in environment
...
function microbe_comp(S::Environment)
	@unpack_Environment S
...
function hubbell_step(C::Communittee)
	@unpack_Communittee C

	mnl = Multinomial(J,N/J)
	N = rand(mnl)
	
	@pack_Communittee! C
	return C # might not need the retrun?
end

function hubbell_loop(sys::System)
	@unpack_System sys
...
# returns post immigration microbe abundance in Environment
function microbe_imm(S::Environment)
	@unpack_Environment S

	# add incoming microbes to Environment abundances
...
function host_acquire(sys::System)
	...
	# microbe abundances in Environment after acquisition by hosts
	S.N -= Na
		...
		if h.age >= floor(Int64,h.L)
			...
			if W > 0
				if t == 0 # environmental transmission
					...
					# host parent microbes go into Environment
					S.N += h.N
				...
				# host microbes go into Environment
				S.N += h.N
			end
